...
public class OLlamaMenu extends AbstractContainerMenu {

    public Container container;
    public com.dragn0007.dragnlivestock.entities.llama.OLlama OLlama;

    public OLlamaMenu(int containerId, Inventory inventory, Container container, OLlama OLlama) {
        ...
        this.addSlot(new Slot(this.container, yakSlots++, 15, 26) {
            ...
            public boolean mayPlace(ItemStack itemStack) {
                return itemStack.is(Items.SADDLE) && !this.hasItem() && OLlama.isSaddleable();
            }

            @Override
            public boolean isActive() {
                return OLlama.isSaddleable();
            }
        });

        if(this.OLlama.isChested()) {
            for(int y = 0; y < 2; y++) {
                for(int x = 0; x < 7; x++) {
    ...
    public boolean stillValid(Player player) {
        return this.OLlama.isAlive() && this.OLlama.distanceTo(player) <= 8f;
    }
    ...
    public static IContainerFactory<OLlamaMenu> create() {
        return (windowId, inv, data) -> {
            int containerSize = data.readInt();
            OLlama OLlama = (OLlama) inv.player.level.getEntity(data.readInt());
            return new OLlamaMenu(windowId, inv, new SimpleContainer(containerSize), OLlama);
        };
    }
}