...
public class OMooshroomMushroomLayer extends GeoLayerRenderer<OMooshroom> {
    public OMooshroomMushroomLayer(IGeoRenderer entityRendererIn) {
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, OMooshroom entityLivingBaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        RenderType renderType = RenderType.entityCutout(((OMooshroom)entityLivingBaseIn).getMushroomsLocation());
