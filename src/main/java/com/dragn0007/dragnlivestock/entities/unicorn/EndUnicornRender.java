...
public class EndUnicornRender extends ExtendedGeoEntityRenderer<EndUnicorn> {
    ...
    public EndUnicornRender(EntityRendererProvider.Context renderManager) {
        ...
        this.addLayer(new EndUnicornCarpetLayer(this));
        this.addLayer(new EndUnicornArmorLayer(this));
    }

    @Override
    public void render(GeoModel model, EndUnicorn animatable, float partialTick, RenderType type, PoseStack poseStack, MultiBufferSource bufferSource, VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
